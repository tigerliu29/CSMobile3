...
export class DocSearchPage {
    ...
    LoadList(name:string) {
      ...
      // else {
        ...
        loader.present();
        this.csdata.DogumentListFileName(this.Path,name)
          .subscribe(
            result => {
              console.log(result);
    ...
    ItemClick(item: ListItem) {
      if (item.Type == "Directory")
        this.navCtrl.push(DocSearchPage, { Path: item.Data.Path, Name: item.Data.Name })
      else {
